    ...
    <div className="fixed inset-0 bg-white z-[80] overflow-y-auto">
      ...
      <div className="max-w-2xl mx-auto p-4 md:p-8">
        <div className="space-y-8">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {paymentMethods.map(method => (
                <button key={method.id} onClick={() => setSelectedPaymentMethod(method.id)} className={`flex flex-col items-center justify-center gap-2 p-4 border rounded-lg h-28 transition-colors ${selectedPaymentMethod === method.id ? "border-purple-500 bg-purple-50" : "border-gray-300 hover:border-gray-400"}`}>
          ...
          <div className="pt-4">
            <Button size="lg" className="w-full bg-gradient-to-r from-purple-600 to-blue-600 text-white hover:from-purple-700 hover:to-blue-700" onClick={handlePayment} disabled={!selectedPaymentMethod || isLoading}>
      ...
      {paymentSuccess && (
        <div className="fixed inset-0 bg-black/60 z-[90] flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-8 text-center space-y-4 max-w-sm shadow-xl">
            <CheckCircle className="h-16 w-16 text-green-500 mx-auto" />
            <h3 className="text-2xl font-bold">Payment Successful!</h3>
            <p className="text-gray-600">Thank you for your purchase. An order confirmation has been sent to your email.</p>
            <Button onClick={() => { setPaymentSuccess(false); onBack(); }} className="w-full bg-gray-800 hover:bg-gray-900 text-white">Continue Shopping</Button>
          </div>
        </div>
      )}
    </div>
